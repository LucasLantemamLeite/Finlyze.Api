...
public class FakeCreateAdminUserAccountHandlerTest
{
    private List<UserAccount> _users = new();

    private FakeCreateAdminUserAccountHandler _handler;

    public FakeCreateAdminUserAccountHandlerTest()
    {
        ...
        _handler = new FakeCreateAdminUserAccountHandler(_users);
    }
    ...
    public async Task Dado_Um_UserAccount_Válido_Adiciona_Na_Entidade()
    {
        ...
        Assert.Equal("Conta do usuário admin criado com sucesso.", result.Message);
        Assert.NotNull(result.Data);
        Assert.True(_users.Any(x => x.Email.Value == command.Email && (x.Role.Value & (ERole.Admin | ERole.User)) == (ERole.Admin | ERole.User)));
    }
