...
public class CreateUserAccountHandler : ICreateUserAccountHandler
{
    ...
    public async Task<ResultHandler<UserAccount>> Handle(CreateUserAccountCommand command)
    {
        try
        {
            ...
            {
                await _appRepository.CreateAsync(new AppLog((int)ELog.Warning, "UserAccount", $"Não foi possível criar conta de usuário: e-mail '{command.Email}' já está em uso."));
            ...
            {
                await _appRepository.CreateAsync(new AppLog((int)ELog.Warning, "UserAccount", $"Não foi possível criar conta de usuário: telefone '{command.PhoneNumber}' já está em uso."));
            ...
            if (rows == 0)
            {
                await _appRepository.CreateAsync(new AppLog((int)ELog.Error, "UserAccount", $"Falha ao criar conta de usuário com e-mail '{command.Email}'."));
                return ResultHandler<UserAccount>.Fail("Falha ao criar conta do usuário.");
            }

            await _appRepository.CreateAsync(new AppLog((int)ELog.Info, "UserAccount", $"Conta de usuário '{userAccount.Email.Value}' criada com sucesso."));
        ...
        catch (Exception ex) when (ex is DomainException or EmailRegexException or PhoneNumberRegexException or EnumException)
        {
            ...
            await _appRepository.CreateAsync(new AppLog((int)ELog.Validation, "UserAccount", $"Erro de validação ao criar conta de usuário com e-mail '{command.Email}': {errorMsg}"));
        ...
        catch (Exception e)
        {
            ...
            await _appRepository.CreateAsync(new AppLog((int)ELog.Error, "UserAccount", $"Erro inesperado ao criar conta de usuário com e-mail '{command.Email}': {errorMsg}"));
